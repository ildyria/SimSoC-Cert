...
(** Structures.OrderedType module for Z *)
...
Module ZOrd.
  ...
  Proof. unfold eq. intros. omega. Qed.
  ...
  Proof.
    intros x y. case_eq (Zcompare x y); intro h.
    eapply EQ. apply Zcompare_Eq_eq. auto.
    eapply LT. apply h.
    eapply GT. unfold lt, Zlt. rewrite <- Zcompare_Gt_Lt_antisym. auto.
  Defined.

  Definition eq_dec := Z_eq_dec. (*: forall x y, { eq x y } + { ~ eq x y }.

  Proof.
    intros x y. case_eq (Zcompare x y); intro h.
    left. apply Zcompare_Eq_eq. auto.
    right. unfold eq. intro. subst. rewrite Zcompare_refl in h. discriminate.
    right. unfold eq. intro. subst. rewrite Zcompare_refl in h. discriminate.
  Defined.*)
...
Module ZMap := FMapAVL.Make ZOrd.

Definition app {A} a (m : ZMap.t A) x :=
  match ZMap.find x m with
    | Some v => v
    | None => a
  end.

Definition app2 {A} a (m : ZMap.t (ZMap.t A)) x y :=
  match ZMap.find x m with
    | Some m' => app a m' y
    | None => a
  end.
...
Module Import ZMapFacts := FMapFacts.Facts ZMap.
...
Definition fupdate {A: Type} (x: Z) (v: A) (f: ZMap.t A) : ZMap.t A :=
  ZMap.add x v f.
...
Lemma fupdate_s:
  forall (A: Type) (a: A) (x: Z) (v: A) (f: ZMap.t A),
  app a (fupdate x v f) x = v.
Proof.
  intros; unfold app, update. rewrite add_o. rewrite zeq_true. auto.
Qed.
...
Lemma fupdate_o:
  forall (A: Type) (a: A) (x: Z) (v: A) (f: ZMap.t A) (y: Z),
  x <> y -> app a (fupdate x v f) y = app a f y.
Proof.
  intros; unfold app, update. rewrite add_neq_o. 2: auto. auto.
Qed.
...
Record mem' : Type := mkmem {
  mem_contents_map: ZMap.t (ZMap.t memval); (*block -> Z -> memval*)
  mem_access: block -> Z -> option permission;
    (*ZMap.t (ZMap.t (option permission));*)
  bounds_map: ZMap.t (Z * Z) (*block -> Z * Z*);
  nextblock: block;
  nextblock_pos: nextblock > 0;
  nextblock_noaccess: forall b, 0 < b < nextblock \/ app (0,0) bounds_map b = (0,0) ;
  bounds_noaccess: forall b ofs, ofs < fst(app (0,0) bounds_map b) \/ ofs >= snd(app (0,0) bounds_map b) -> (*app2 None*) mem_access(*_map*) b ofs = None;
  noread_undef: forall b ofs,  perm_order' ((*app2 None*) mem_access(*_map*) b ofs) Readable \/ app2 Undef mem_contents_map b ofs = Undef
}.

Definition mem := mem'.

Definition mem_contents m := app2 Undef (mem_contents_map m).

(*REMOVE:Definition mem_access m := app2 None (mem_access_map m).*)

Definition bounds m := app (0,0) (bounds_map m).

Ltac r := unfold mem_contents, (*mem_access,*) bounds.

Tactic Notation "r" "in" hyp(h) :=
  unfold mem_contents, (*mem_access,*) bounds in h.
...
Proof.
  ...
  intros.
  (*r in H.*) rewrite H0 in H. contradiction.
  destruct (zlt ofs (snd (bounds m b))).
  omega. 
  exploit bounds_noaccess. right; eauto.
  intro. (*r in H.*) rewrite H0 in H. contradiction.
Qed.
...
Program Definition empty: mem :=
  mkmem (@ZMap.empty _) (*fun b ofs => Undef*)
        (*@ZMap.empty _*) (fun b ofs => None)
        (@ZMap.empty _) (*fun b => (0,0)*)
        1 _ _ _ _.
Next Obligation.
  omega.
Qed.
...
Program Definition alloc (m: mem) (lo hi: Z) :=
  (mkmem (fupdate m.(nextblock) 
                 (@ZMap.empty _)(*fun ofs => Undef*)
                 m.(mem_contents_map))
         (update m.(nextblock)
                 (fun ofs => if zle lo ofs && zlt ofs hi then Some Freeable else None)
                 m.(mem_access))
         (fupdate m.(nextblock) (lo, hi) m.(bounds_map))
         (Zsucc m.(nextblock))
         _ _ _ _,
   m.(nextblock)).
Next Obligation.
...
(*  generalize (bounds_noaccess m b 0).*)
  ...
  rewrite fupdate_o. right; auto. omega.
Qed.
Next Obligation.
  unfold update in *. destruct (zeq b (nextblock m)). 
  subst b. rewrite fupdate_s in H. simpl in H. destruct H. 
  unfold proj_sumbool. rewrite zle_false. auto. omega.
  unfold proj_sumbool. rewrite zlt_false; auto. rewrite andb_false_r. auto.
  apply bounds_noaccess. rewrite fupdate_o in H; auto.
Qed.
Next Obligation.
unfold update.
destruct (zeq b (nextblock m)).
unfold app2, fupdate. rewrite add_eq_o. 2: auto.
unfold app. rewrite empty_o. auto.
unfold app2, fupdate. rewrite add_neq_o. 2: auto.
apply noread_undef.
Qed.

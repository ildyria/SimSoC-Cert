...
Section range_dec.
  ...
  Proof.
    ...
    rewrite nat_of_Z_eq in zb. 2: auto. omega.
    rewrite nat_of_Z_neg in zb. 2: omega. simpl in zb. omega.
    (* <- *)
    intros z [za zb]. apply h. destruct (Z_ge_dec (b-a) 0).
    rewrite nat_of_Z_eq. 2: auto. omega.
  ...
  Proof.
    ...
    right. intro h. absurd (P(a+Z_of_nat k)). auto.
    apply h. rewrite inj_S. omega.
    right. intro h. absurd (ranged_aux a k). auto.
  ...
  Proof.
    ...
    left. rewrite ranged_eq. auto.
    right. rewrite ranged_eq. auto.
  Defined.
...
Proof.
  ...
  left; red; intros. omegaContradiction.*)
  intros m b lo hi p.
  set (P := fun ofs => perm m b ofs p).
  assert (Pdec : forall ofs, {P ofs}+{~P ofs}).
  intro ofs. unfold P. apply perm_dec.
  destruct (ranged_dec P Pdec lo hi).
  left. intros ofs h. apply r. auto.
  right. intro h. absurd (ranged P lo hi). auto.
  intros ofs H. apply h. auto.
Defined.
